function [FIG1,FIG2] = plot_sensitivity(funPath, name, ind1,ind2, labels, c)
...
% Pretty colours
colours = colour_pairs(c);
b(1).FaceColor = colours{1};
b(2).FaceColor = colours{2};

% Save figure full-screen
saveas(FIG1,strcat(funPath,'sobol_sens_', name),'epsc')
...
% saveas(FIG2,strcat(funPath,'sobol_imp_', name),'epsc')
saveas(FIG2,strcat(funPath,'sobol_imp_', name),'pdf')
