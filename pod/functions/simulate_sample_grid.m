...
for j = 1 : size(sample, 1)
    ...
    tic;
    [time_full,x_full(:,:,j)] = RK3 (@(t,x) circadian_rhs(t,x,params,tau),[0 time.final_time], time.x0, time.dt );
    elapsed_time_full(j)=toc;
    [synchrony_full(:,j),sync_param_full(j),avg_gene_conc_full(:,j),spectral_ampl_fact_full(j),estimated_period_full(j),order_param_X_full(:,j)] = compute_QoIs(time_full,x_full(:,:,j),num_n,time.n_asympt,params.omega,params.L0);
    
    % Reduced order model (ROM) : POD
    tic;
    [time_rom,x_rom_proj] = RK3 (@(t,x) U_pod'*circadian_rhs(t,U_pod*x,params,tau),[0 time.final_time], U_pod'*time.x0, time.dt);
    x_rom(:,:,j)=x_rom_proj*U_pod';
    elapsed_time_rom(j)=toc;
    [synchrony_rom(:,j),sync_param_rom(j),avg_gene_conc_rom(:,j),spectral_ampl_fact_rom(j),estimated_period_rom(j),order_param_X_rom(:,j)] = compute_QoIs(time_rom,x_rom(:,:,j),num_n,time.n_asympt,params.omega,params.L0);
        
    % Parametric reduced order model (pROM) : POD
    tic;
    [time_prom,x_prom_proj] = RK3 (@(t,x) V_pod'*circadian_rhs(t,V_pod*x,params,tau),[0 time.final_time], V_pod'*time.x0, time.dt);
    x_prom(:,:,j)=x_prom_proj*V_pod';
    elapsed_time_prom(j)=toc;
    [synchrony_prom(:,j),sync_param_prom(j),avg_gene_conc_prom(:,j),spectral_ampl_fact_prom(j),estimated_period_prom(j),order_param_X_prom(:,j)] = compute_QoIs(time_prom,x_prom(:,:,j),num_n,time.n_asympt,params.omega,params.L0);
    ...
    tic;
    [time_deim,x_deim_proj]=RK3(@(t,x) PTUinv*circadian_rhs_colloc(t,x,params,PTtau,PTU,meanU_v), [0 time.final_time], U_pod'*time.x0, time.dt);
    x_deim=x_deim_proj*U_pod';
    elapsed_time_deim(j)=toc;
    [synchrony_deim(:,j),sync_param_deim(j),avg_gene_conc_deim(:,j),spectral_ampl_fact_deim(j),estimated_period_deim(j),order_param_X_deim(:,j)] = compute_QoIs(time_deim,x_deim,num_n,time.n_asympt,params.omega,params.L0);
