...
colours_rnked(idx(1:floor(N/3)), :) = colour_groups('blue', length(1:floor(N/3)));
colours_rnked(idx(ceil(N/3):floor(2*N/3)), :) = colour_groups('green', length(ceil(N/3):floor(2*N/3)));
colours_rnked(idx(ceil(2*N/3):end), :) = colour_groups('red', length(ceil(2*N/3):N));
...
saveas(FIG,strcat(method, '/', output, '/', name),'epsc')
