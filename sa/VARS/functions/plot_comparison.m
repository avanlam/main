...
sgtitle(strcat("\textbf{Sobol's ", name, " according to different model outputs}"), 'FontSize', 45);
...
for i = 1:2
    ...
    subplot(2,1,i)
    plot(repmat(x, 2,1), [min(c_cell{i}, [],2), max(c_cell{i}, [],2)]', 'Color', colours{1}, 'LineWidth', 4); hold on;
    plot(x, y, 'o', 'Color', colours{2}, 'MarkerSize', 12);
    if i==1
        y = round(y,2);
    end
...
sgtitle(strcat("\textbf{Sobol's ", name, " variation across the three model outputs}"), 'FontSize', 45);
